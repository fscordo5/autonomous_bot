...
void SensorEnable::CmdvelCallBack(const geometry_msgs::Twist& msg){
    ...
    if(turning_radius < -MAX_STEER_radius || turning_radius > MAX_STEER_radius){
        vec_servo_[0].call(srv_zero);
    }
    else if(turning_radius > -MIN_STEER_radius && turning_radius < MIN_STEER_radius){
        ROS_ERROR ("The given radius (%f m) is smaller than the minimum value", turning_radius);
        ROS_INFO("linear_vel: %f , angular_vel: %f" , linear_vel,  angular_vel );
    }
    else {
        for (int i = 0; i < radius_list.size(); i++){
            if(turning_radius >= radius_list[i] && turning_radius < radius_list[i+1]){
                srv_act.request.value = angle_list[i]+((turning_radius-radius_list[i])/(radius_list[i+1]-radius_list[i]))*(angle_list[i+1]-angle_list[i]);
            }
        }
        vec_servo_[0].call(srv_act);
    }
}
...
void SensorEnable::Initialize_sensors(){
    ...
    for (auto actuator = motors.begin(); actuator != motors.end(); ++actuator){
     ...
     for (auto actuator = servos.begin(); actuator != servos.end(); ++actuator){
        ...
        vec_servo_.push_back(nh_.serviceClient<webots_ros::set_float>(SensorEnable::robot_name_+ *actuator +"/set_position"));
        ros::service::waitForService(SensorEnable::robot_name_ + *actuator + "/set_position");
        vec_servo_.back().call(srv_zero);
        //vec_servo_.push_back(vec_client.back());
